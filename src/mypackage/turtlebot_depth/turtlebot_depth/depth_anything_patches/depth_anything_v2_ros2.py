...
class DepthAnythingROS(Node):
    ...
    def __init__(self):
        ...
        # Create common publishers with RELIABLE to match typical ROS2 image subscribers
        ...
        self.depth_image_pub = self.create_publisher(
            Image, self.depth_image_topic, publisher_qos_profile)

        # Create subscribers with RELIABLE to match camera publisher
        ...
        self.image_sub = self.create_subscription(
            Image, self.image_topic, self.image_callback, subscriber_qos_profile)
    ...
    def image_callback(self, image_msg: Image) -> None:
        ...
        # Log once when first image is received
        self.get_logger().info(
            f'Publishing depth images to: [{self.depth_image_topic}]', once=True)
        ...
        try:
            ...
            self.get_logger().info(f"Published depth image", throttle_duration_sec=2.0)
        except CvBridgeError as e:
            print(e)
